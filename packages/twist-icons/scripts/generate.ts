...
import { dirname, allIconsDir } from './constants'
import {
  mkdirSync,
  getTemplate,
  getTSConfig,
...
async function compileTS(config: TSConfig, configPath, outpath) {
  ...
  const { stderr } = await execa('npx', ['tsc', '-p', tsConfigPath])
  if (stderr) {
    print.error(stderr)
  }
...
async function generateIcons() {
  ...
  for (const IconConfig of config) {
    ...
    const outputPath = path.resolve(allIconsDir, `${iconSetName}-icons`)
